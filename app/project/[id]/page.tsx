    ...
    export default function ProjectPage({ params }: { params: { id: string } }) {
      ...
      useEffect(() => {
        const fetchStatus = async () => {
          try {
            ...
            // Fetch SVG preview when ready
            if (data.project.status === 'ready' && data.hasAssets.svg && !svgContent) {
              try {
      ...
      return (
        <div className="max-w-2xl mx-auto px-4 py-16">
          ...
          {/* SVG Preview */}
          {project.status === 'ready' && hasAssets.svg && svgContent && (
            <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
              <h2 className="text-xl font-semibold mb-4">Flat Sketch Preview</h2>
              <div
                className="w-full flex items-center justify-center bg-gray-50 rounded-lg p-4 min-h-[300px]"
